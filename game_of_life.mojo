    ...
    while running:
        ...
        if event.type == pygame.QUIT:
            ...
            if Bool(event.key == pygame.K_ESCAPE)
                or Bool(event.key == pygame.K_q):
                running = False
        
        # Clear the window by painting with the background color
        window.fill(background_fill_color)

        # Draw each populated cell in the grid
        for row in range(grid.rows):
            for col in range(grid.cols):
                if grid[row, col]:
                    x = col * cell_width + boder_size
                    y = row * cell_height + boder_size
                    width = cell_width - boder_size
                    height = cell_height - boder_size
